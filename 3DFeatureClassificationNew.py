...
path = "/home/peteng/data/Facial Features/"
...
joblib.dump(regr, "RFC Model 3D Features") 
#regr2 = joblib.load("RFC Model 3D Points")
print("is 3d test any nan:", np.nan_to_num(threeD_file_test_transformed))
print("is 3d test any inf:", np.nan_to_num(threeD_file_test_transformed))
print("3d file test transformed:", threeD_file_test_transformed)



predictionRegr = regr.predict(np.nan_to_num(threeD_file_test_transformed))
...
predictprobRFC = regr.predict_proba(np.nan_to_num(threeD_file_test_transformed))
print("---RFC Training and Prediction Done :)---")

f=open(r"threeDPointsOutput.txt", "a")
f.write("############3D Points Features Metrics##########")
f.write("############RFC Metrics############")
...
joblib.dump(xgb_model, "XGB model 3D Points") 
#xgb_model2 = joblib.load("XGB Model 3D Points")
...
predictprobXGB = xgb_model.predict_proba(np.nan_to_num(threeD_file_test_transformed))
...
joblib.dump(gnb, "GNB model 3D Points") 
#gnb2 = joblib.load("GNB Model 3D Points")
...
predictprobGNB = gnb.predict_proba(np.nan_to_num(threeD_file_test_transformed))
...
joblib.dump(clf, "SVM Model 3D Points") 
...
predictprobSVM = clf.predict_proba(np.nan_to_num(threeD_file_test_transformed))
