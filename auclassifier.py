...
path = "/home/peteng/data/Facial Features/Action Units/"
...
predictionRegr = regr.predict(np.nan_to_num(au_file_test_transformed)) 
...
predictprobRFC = regr.predict_proba(np.nan_to_num(au_file_test_transformed))
print("---RFC Training and Prediction Done :)---")

f=open(r"auOutput.txt", "a")
f.write("############Action Unit Features Metrics##########")
f.write("############RFC Metrics############")
...
joblib.dump(clf, "SGC SVM model AU") 
#clf2 = joblib.load("SVM Model")
#print("SVM model loaded")
predictionSVM = clf.predict(np.nan.to.num(au_file_test_transformed))  
...
predictprobSVM = clf.predict_proba(np.nan_to_num(au_file_test_transformed))
...
svmauc2sum = np.add.reduceat(svmauc2instances, AU_indexes[:-1],0)
...
joblib.dump(xgb_model, "XGB Classifier Model AU") 
#xgb_model2 = joblib.load("XGB Classifier Model")
print("XGB model saved")

predictionXGB = xgb_model.predict(np.nan_to_num(au_file_test_transformed))
...
predictprobXGB = xgb_model.predict_proba(np.nan_to_num(au_file_test_transformed))
...
joblib.dump(gnb, "GNB model AU") 
#gnb2 = joblib.load("GNB Model")
print("GNB model saved")
predictionGNB = gnb.predict(np.nan_to_num(au_file_test_transformed))
predictionGNBSum = np.add.reduceat(predictionGNB,AU_indexes[:-1],0).reshape(-1,1)
...
predictprobGNB = gnb.predict_proba(np.nan_to_num(au_file_test_transformed))
