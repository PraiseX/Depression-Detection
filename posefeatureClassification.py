...
path = "D:\MQP\dataset\DAICWOZ\Facial Features\\Pose\\"
...
joblib.dump(regr, "RFC Model pose") 
...
predictprobRFC = regr.predict_proba(np.nan_to_num(pose_file_test))
print("---RFC Training and Prediction Done :)---")

print("############Pose Features Metrics############", file=open('output.txt', 'a'))
...
joblib.dump(xgb_model, "XGB model pose") 
...
predictprobXGB = xgb_model.predict_proba(np.nan_to_num(pose_file_test))
...
joblib.dump(gnb, "GNB model pose") 
...
predictprobGNB = gnb.predict_proba(np.nan_to_num(pose_file_test))
...
joblib.dump(clf, "SVM Model pose") 
...
predictprobSVM = clf._predict_proba_lr(np.nan_to_num(pose_file_test))
